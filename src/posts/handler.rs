...
pub async fn get_posts_handler(
    ...
    let posts = sqlx::query_as::<_, Post>("select * from posts")
        ...
        .map_err(|e| {
            let response =
                serde_json::json!({ "status": "fail","message": format!("Database error: {}", e) });
...
pub async fn get_post_by_id_handler(
    ...
    let posts = sqlx::query_as::<_, Post>("select * from posts where id = $1")
        .bind(id)
        .fetch_one(&state.db)
        .await
        .map_err(|e| {
...
pub async fn create_posts_handler(
    ...
    let post = sqlx::query_as::<_,Post>(
        ...
        .await
        .map_err(|e|{
            let response = serde_json::json!({ "status": "fail","message": format!("Database error: {}", e) });
            (StatusCode::INTERNAL_SERVER_ERROR,Json(response))
        })?;
...
pub async fn get_my_posts_handler(
    Extension(user): Extension<User>,
    State(state): State<AppState>,
) -> Result<impl IntoResponse, (StatusCode, Json<serde_json::Value>)> {
